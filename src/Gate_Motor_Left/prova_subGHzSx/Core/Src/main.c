...
uint8_t fotoc = 0;        	//serve per la logica della fotocellula (si alzerà quando mi arriva la interrupt dell'ultrasuono
uint8_t messaggio_scartato = 0;

uint16_t pwm_attuale = 1800;
uint16_t pwm_bersaglio = 1800;

//variabili usate per il controllo degli ultrasuoni
volatile uint16_t tempo_ritorno_eco = 0;
volatile uint8_t eco_ricevuto = 0;
uint8_t turno_sensore = 0;
...
static void MX_TIM4_Init(void)
{
  ...
  /* Imposta Prescaler e Period per ottenere 30 secondi a 48MHz */
